...
// loader when the spec names one, a LoRA loader when the spec names one, and
// — new here — baking the pixel-grid downscale/upscale into the graph itself
// as real ImageScale nodes, since the spec asks for them by name rather than
// the client-side pixelate_image post-process the rest of PixelForge uses.
// ---------------------------------------------------------------------------
...
export function buildWorkflowFromSpec(spec: PromptSpec, checkpoint: string): BuiltSpecWorkflow {
  ...
  if (spec.lora) {
    insertLora(workflow, spec.lora, "Spec LoRA");
  }
