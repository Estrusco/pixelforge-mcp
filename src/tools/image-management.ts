...
import { AssetRegistry, isLocalAsset, type AssetRecord } from "../services/asset-registry.js";
import {
...
export function registerImageManagementTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "Fetch, browse and inspect ComfyUI images and registered assets. Driven by the `action` parameter:\n" +
      ...
      '- action:"analyze_color" — Measure the color of a rendered image (not by eye): returns black/white points, contrast (luma std), saturation, per-channel means + cast, and clipping — plus heuristic flags (washedOut, lowContrast, liftedBlacks, dimHighlights, lowSaturation, colorCast) and a one-line verdict. For an image with an alpha channel (e.g. a remove_background/pixelate_image cutout), also returns an alpha section (transparentPct/semiTransparentPct/opaquePct/meanAlpha) — use it to confirm a cutout\'s edges/glow are actually soft instead of eyeballing a checkerboard preview; omitted when the source has no alpha channel. Source = asset_id, a ComfyUI output ref (filename/subfolder/type), or an image path. Pass reference_path to shot-match against a known-good frame (target−reference deltas). Set histogram:true to also get an overlaid R/G/B/luma histogram PNG. Use this to diagnose \'washed out\' objectively and decide a color fix; for a video, extract a frame to PNG first.\n' +
      ...
      save_dir: z
        ...
        .describe(
          'action:"get" — absolute local directory to save the file in. Defaults to a ' +
            ...
            'action:"asset_metadata" — optional (no default; omit to skip persisting): an ' +
            "arbitrary local directory (not required to be under COMFYUI_PATH, e.g. a folder " +
            "inside your game project) to also write this asset's workflow snapshot to, as " +
      ...
      try {
        ...
        switch (args.action) {
          ...
          case "asset_metadata": {
            ...
            const savedTo = args.save_dir
              ? await saveWorkflowSnapshot(record, args.save_dir)
              : undefined;
            return json({
