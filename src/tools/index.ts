...
const TOOL_GROUPS: ReadonlyArray<readonly [category: string, register: (server: McpServer) => void]> = [
  ...
  // remove_background is locked PixelForge tool #6 (tool-surface.md) — upstream's
  // 0.50.0 slice 16 folded its OWN copy into generate_image (action:"remove_background"),
  // but our fork keeps the standalone tool (asset_id/path + luma_key widening) per
  // the locked decision; removeBackgroundAction still backs the folded upstream
  // action too, so both call forms share one implementation. Appended (not restored
  // to its pre-sync "generation" slot) for the same tools/list-order reason as above.
