...
export const MUTATING_TOOLS = [
  ...
  "upload_image",
  "workspace",
  // PixelForge additions — writes to ComfyUI's user library, same as save_workflow
  // (which it's a prompt-spec-driven alternative to; it never enqueues/runs).
  "workflow_from_prompt_spec",
  // Writes/removes a persistent credential, same footing as get_defaults action:"set".
  "set_secret",
  "clear_secret",
];
...
export const EXECUTION_TOOLS = [
  ...
  "report_issue",
  // PixelForge additions — each enqueues a ComfyUI render (spends GPU time) or writes
  // a generated file to a caller-chosen local directory, same footing as get_image
  // action:"convert" above.
  "generate_sprite",
...
export const READONLY_TOOLS = [
  ...
  "search_custom_nodes",
  "visualize_workflow",
  // PixelForge additions — pure reads: a job-status poll, a static template dump, a
  // masked-status credential listing, and a local composite with no ComfyUI job or
  // persistent write.
  "get_sprite_result",
