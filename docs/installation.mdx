...
Some tools use API tokens. The easiest way to set one is the `set_secret` tool once
you're connected (e.g. `set_secret {"slot": "civitai", "value": "..."}` — call
`get_secrets` first to see every slot and its current masked status). Tokens can
also be set directly in the server's `env` block (see [Configuration](/configuration)),
or in `~/.comfyui-mcp/.env`:
