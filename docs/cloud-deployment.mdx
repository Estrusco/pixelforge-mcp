...
`COMFYUI_AUTH_HEADER` / `COMFYUI_AUTH_SCHEME`) on the local `connect` command. You can also
save these gateway keys through the secure `panel_request_secret` path for the built-in
`comfyui` server; subsequent ComfyUI/Manager requests re-read them without an orchestrator
restart or `panel_reload`. For a pod fronted by **Cloudflare Access**, create an Access
**service token** and set `CF_ACCESS_CLIENT_ID` + `CF_ACCESS_CLIENT_SECRET` — both ride every
ComfyUI request (HTTP + the queue-watcher WebSocket), so the connector passes the gate while
the human sign-in page stays up for browsers.
</Note>
